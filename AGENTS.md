...
## Backup/Deploy Shortcuts

### Development (local testing)

- While working: use `npm run dev` to test at localhost:5173 (or configured port)
- Build stays in project `dist/` folder — no copying needed for local dev

### `deploy` — Production deploy
When user types "deploy", perform these steps:
1. **Update version string** - In `src/components/auth/WelcomePage.tsx`, change the version string (line ~464: `v{...}`) to a unique identifier based on the most recent changes (e.g., "load25-fix" or "relay-5sec") in the same way that a git commit summary information message would be created
2. **Git commit** - Stage and commit all changes with that descriptive message
3. **Source zip** - Create `/home/q4/corkboards.me-latest.zip` with all source code files needed to completely restore the project in the case of critical failure.  Exclude node_modules, .git
4. **Build** - Run `npm run build` (outputs to project `dist/` folder)
5. **Copy to deploy folder** - Copy dist contents to `/home/q4/dist/`:
   - `rm -rf /home/q4/dist && cp -r /home/q4/corkboard/dist /home/q4/dist`

### `stage` — Staging deploy
When user types "stage", perform these steps:
1. **Update version string** - In `src/components/auth/WelcomePage.tsx`, change the version string (line ~464: `v{...}`) to a unique identifier based on the most recent changes (e.g., "load25-fix" or "relay-5sec") in the same way that a git commit summary information message would be created
2. **Git commit** - Stage and commit all changes with that descriptive message
3. **Source zip** - Create `/home/q4/corkboards.me-latest-stage.zip` with all source code files needed to completely restore the project in the case of critical failure. Exclude node_modules, .git
4. **Build** - Run `npm run build` (outputs to project `dist/` folder)
5. **Copy to deploy folder** - Copy dist contents to `/home/q4/dist_stage/`:
   - `rm -rf /home/q4/dist_stage && cp -r /home/q4/corkboard/dist /home/q4/dist_stage`

### Architecture notes

- **Local dev**: `npm run dev` serves from project `dist/` at localhost
- **Production**: Files in `/home/q4/dist/` → copied to DreamHost root (corkboards.me)
- **Staging**: Files in `/home/q4/dist_stage/` → copied to DreamHost subdomain (stage.corkboards.me)
- Both production and staging use `base: '/'` in vite.config.ts since each has its own origin

### Server files

- `rss-proxy.php` lives in the repo at `/public/rss-proxy.php` and is deployed with both production and staging builds. It provides CORS proxying for RSS feeds (hardcoded to https://corkboards.me/rss-proxy.php in `src/lib/feedUtils.ts`).

## Current Task: Corkboard Feed Loading System

### Goal

Nostr-based "corkboard" app

### Instructions

- **Don't make major architectural changes unless specifically requested**
- **Never auto-fetch relays** unless user explicitly clicks a button
- **Preserve cypherpunk values**
- **Be aware of tech debt and avoid it**
- **Optimize for performance**
- **Don't make changes outside of the scope of the prompted instructions**
