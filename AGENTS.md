...
> **Scope.** This file describes *this project's* scaffolding — components, hooks, file layout, conventions. For Nostr **protocol** guidance (current NIPs, deprecations, relay strategy, kind selection), see **`SKILL.md`** at the repo root. Where the two disagree, `SKILL.md` wins — it is fact-checked against NIP-01 and the live NIPs index. Where `SKILL.md` disagrees with the `mcp__nostr__*` tools or nostrbook.dev, those win.
>
> This file was generated by MKStack scaffolding. Parts of it predate current protocol state; the protocol sections have been corrected against NIP-01, but treat any protocol claim here as secondary.
...
- `/src/contexts/`: React context providers (AppContext, NWCContext, DMContext)
  - `useDMContext`: Hook exported from DMContext for direct messaging (NIP-17; legacy NIP-04 read-only)
...
- **`docs/NOSTR_DIRECT_MESSAGES.md`**: Read when implementing direct messaging features, building chat interfaces, or working with encrypted peer-to-peer communication (NIP-17; NIP-04 is deprecated and read-only here).
...
These are the **exact** NIP-01 rules. The carve-outs are commonly omitted, which is why people wrongly conclude kinds 4–44 aren't stored — that range covers DMs, chat, threads, pictures, video, and public messages.

- **Regular Events** (`1000 ≤ kind < 10000` **or** `4 ≤ kind < 45` **or** `kind == 1` **or** `kind == 2`): Expected to be stored by relays permanently. Used for persistent content like notes, articles, etc.
- **Replaceable Events** (`10000 ≤ kind < 20000` **or** `kind == 0` **or** `kind == 3`): Only the latest event per pubkey+kind combination is stored. Used for profile metadata, contact lists, etc.
- **Ephemeral Events** (`20000 ≤ kind < 30000`): Not stored by relays; forwarded to live subscribers only.
...
Do not assume "kinds below 1000 are legacy" — the rules above are exact. Kind `1` and `2` are regular; kinds `0` and `3` are replaceable. `kind` is an integer 0–65535. For replaceable events sharing a `created_at`, the **lowest id in lexical order** is retained.
...
- `naddr1`: **addressable event coordinates** - For addressable events (kind 30000-39999). "Parameterized replaceable" is the retired name for these
...
### Direct Messaging (NIP-17)

**Use NIP-17 for all new work.** NIP-04 is marked **unrecommended** in the NIPs repo and is deprecated in favor of NIP-17 — it leaks message length and has no versioning. NIP-04 support exists here only to read legacy history; never write it, and never offer it as a user-facing choice.

NIP-17 wraps messages (kinds 14/15) in a NIP-59 gift wrap (kind 1059) with a NIP-13 seal, which hides sender and metadata. Publish to the recipient's **kind 10050** DM relay list, not their general kind 10002 relays. Encryption is **NIP-44** versioned payloads.

The project includes a complete direct messaging system with real-time updates and encrypted storage. **The system is disabled by default** - enable it by passing `enabled: true` in the `DMProvider` config.
