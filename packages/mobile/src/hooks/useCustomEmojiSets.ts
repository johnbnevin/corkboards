/**
 * NIP-30 Custom Emoji Sets (kind 30030) — mobile version.
 *
 * Mirror of packages/web/src/hooks/useCustomEmojiSets.ts.
 * Fetches the user's own emoji sets, any sets referenced in their
 * kind 10030 (emoji favorites) list, and the hardcoded default set.
 */
...
export function useCustomEmojiSets() {
  ...
  const { data: rawEvents, isLoading } = useQuery({
    queryKey: ['custom-emoji-sets', pubkey],
    queryFn: async ({ signal }) => {
      if (!pubkey) return { sets: [], favorites: null };
  ...
  // Collect followed set addresses from kind 10030
  ...
  const { data: followedSets } = useQuery({
    ...
    queryFn: async ({ signal }) => {
      ...
      const filters = followedSetAddresses.map(addr => {
        const [, addrPubkey, dTag] = addr.split(':');
        return { kinds: [30030 as number], authors: [addrPubkey], '#d': [dTag] };
      });
