 ...
 *
 * Port of packages/web/src/hooks/useNostrCustomFeedsSync.ts for mobile.
 ...
 * both), so nobody's corkboards vanish on upgrade. Web mirrors this exactly —
...
export function useNostrCustomFeedsSync() {
  ...
  const load = useCallback(async (): Promise<string | null> => {
    ...
    try {
      return await decryptFromSelf(best.content, wrappedKey, signerMethod, signer, pubkey);
    } catch {
      return null;
    }
  }, [pubkey, signer]);
