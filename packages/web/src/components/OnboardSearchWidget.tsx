...
export function OnboardSearchWidget({ contactCount = 0, followTarget = 10, onSkip }: { contactCount?: number; followTarget?: number; onSkip?: () => void }) {
  ...
  const handleInput = useCallback((value: string) => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      setIsSearching(true)
      try {
        // Fresh (uncached) instance — this widget closes the relay on timeout/
        // cleanup, and closing a shared cached relay would poison it for others.
        const relay = createRelayFresh(NIP50_SEARCH_RELAY, { backoff: false })
