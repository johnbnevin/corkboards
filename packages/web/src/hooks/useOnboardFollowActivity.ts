...
/**
 * During onboarding, fetches reactions (kind 7), reposts (kind 6),
 * and replies (kind 1 with e-tags) from the user's current follows.
 * Returns up to ~60 recent activity notes.
 *
 * Auto-fetch runs once when first enabled (contacts loaded). After that,
 * new notes are only added when the caller invokes `fetchNow()` — this lets
 * the user control discovery via a manual "Find more for me" button instead
 * of having the feed refresh automatically.
 */
export function useOnboardFollowActivity(
    ...
    try {
      ...
      for (let i = 0; i < pubkeys.length; i += batchSize) {
        ...
        for (const ev of events) {
          ...
          // Kind 1: only include replies (has e-tags)
  ...
  // Fetch once when first enabled and contacts are available
  useEffect(() => {
