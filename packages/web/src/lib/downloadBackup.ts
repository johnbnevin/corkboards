/**
 * Settings backup download, restore, and checkpoint system.
 *
 * - Download: identical to Blossom backup (same BACKED_UP_KEYS, v4 format with stats)
 * - Checkpoints: last 3 restores saved locally so user can revert if a bad restore
 *   (e.g., empty backup) overwrites good data.
...
function countArr(json: string | null | undefined): number {
  ...
  try {
    ...
    if (import.meta.env.DEV) console.warn('[downloadBackup] countArr parse error:', e);
    return 0;
  }
}
...
function corkboardNames(json: string | null | undefined): string[] {
  ...
  try {
    ...
    if (import.meta.env.DEV) console.warn('[downloadBackup] corkboardNames parse error:', e);
    return [];
  }
}
...
/** Download settings as a JSON file (identical content to Blossom backup). */
export async function downloadSettingsBackup(): Promise<void> {
  ...
  (backup as Record<string, unknown>).checksum = hashArray.map(b => b.toString(16).padStart(2, '0')).join('');

  const json = JSON.stringify(backup, null, 2);
  const blob = new Blob([json], { type: 'application/json' });
  const url = URL.createObjectURL(blob);
  const date = new Date().toISOString().slice(0, 10);
  triggerDownload(url, `corkboards-backup-${date}.json`);
  URL.revokeObjectURL(url);

  idbSetSync(LAST_LOCAL_DOWNLOAD_KEY, String(Date.now()));
}
...
export function shouldPromptBackupDownload(): boolean {
  const lastTs = idbGetSync(LAST_LOCAL_DOWNLOAD_KEY);
  if (!lastTs) {
    idbSetSync(LAST_LOCAL_DOWNLOAD_KEY, String(Date.now()));
    return false;
  }
...
export function getCheckpoints(): Checkpoint[] {
  const raw = idbGetSync(STORAGE_KEYS.RESTORE_HISTORY);
  if (!raw) return [];
  try {
...
export function saveCheckpoint(source: 'file' | 'nostr', name?: string): void {
  ...
  idbSetSync(STORAGE_KEYS.RESTORE_HISTORY, JSON.stringify([checkpoint, ...existing]));
}
...
export function renameCheckpoint(index: number, name: string): void {
  ...
  if (index >= 0 && index < checkpoints.length) {
    ...
    idbSetSync(STORAGE_KEYS.RESTORE_HISTORY, JSON.stringify(checkpoints));
  }
}
...
/** Restore settings from a backup JSON file. Returns number of keys restored.
 *  Awaits all IDB writes so data persists reliably.
 *  Supports both v4 format ({version, settings: {...}}) and flat format ({key: value, ...}). */
export async function restoreFromBackupFile(json: string): Promise<number> {
