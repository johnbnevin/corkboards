...
async function _fetchEventWithOutboxImpl(
  ...
  opts?: {
     ...
     *  Tried in order after authorPubkey's relays miss — see phase 2. */
  ...
  // thread participants. Why the walk: a reply's e-tag often omits the parent
  // author (no NIP-10 pubkey field — Wisp, among others), and its FIRST p-tag
  // is frequently the wrong person (the replier itself, or the thread root's
  // author). NIP-10 only guarantees the parent's author is SOMEWHERE in the
  // p-tags. An event that lives solely on its author's own relays — none of
  // them in our fallback set — is unfindable until the RIGHT author's outbox
  // is consulted, so when the primary author's relays miss, try the others.
  // Bounded: 3 authors total, primary gets 3 relays, candidates 2 each, and
  // relays already covered by phase 1 are skipped.
...
export function fetchEventWithOutbox(
  eventId: string,
  nostr: NostrLike,
  opts?: { onRelayTried?: (r: string) => void; hints?: string[]; authorPubkey?: string; candidateAuthors?: string[] },
): Promise<NostrEvent | null> {
 ...
 * lookup (pool + hints + author/candidate outboxes), and if that still misses,
 * a parallel sweep of LAST_RESORT_LOOKUP_RELAYS — archives, indexers and the
 * big general relays — each queried directly with a generous deadline.
 *
 * Deliberately NOT used by any automatic path (first pass, sweep, second
 * pass): a human clicked, so one burst of ~8 extra sockets is proportionate;
 * on a timer it would be exactly the socket-budget churn the governor exists
 * to prevent.
 */
export async function fetchEventLastResort(
  ...
  // Everything phase 1 already covered — don't pay for the same relay twice.
